...
# from colabfpnmgn.MGN import MGN
#  from colabpapergo.MGN import MGN
...
def train():
    ...
    # print("len(imdb):----------%d------------" % len(imdb))
    # print("len(ratio_list):----------%d------------" % len(ratio_list))
    # print("ratio_list:----------%d------------" % ratio_list)
    # print("len(ratio_index):----------%d------------" % len(ratio_index))
    # print("ratio_index:----------%d------------" % ratio_index)
    # print("len(ratio_index):----------%d------------" % len(ratio_index))
    # print("ratio_index:----------%d------------" % ratio_index)
    # print("len(cls_list):----------%d------------" % len(cls_list))
    # print("cls_list:----------%d------------" % cls_list)

    train_size = len(roidb)
    print('{:d} roidb entries'.format(len(roidb)))
    sampler_batch = sampler(train_size, args.batch_size)
    iters_per_epoch = int(train_size / args.batch_size)  # 一个epoch中含有批次的数量，和args.iter_per_epoch一样
    if args.frame in {"fpn", "faster_rcnn", "efc_det"}:
        dataset = fasterrcnnbatchLoader(roidb, ratio_list, ratio_index, args.batch_size, \
    ...
    # args.iter_per_epoch：一个epoch中含有批次的数量
    args.iter_per_epoch = int(len(roidb) / args.batch_size)
    print("args.iter_per_epoch: -------%d---------" % args.iter_per_epoch)

    # init output directory for model saving
    # 初始化训练模型保存路径
    output_dir = args.save_dir + "/" + args.dataset + "/" + args.net
    print("output_dir,模型保存路径： -------%s -------" % output_dir)
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    if args.vis:
        visualizer = dataViewer(cls_list)  # 一个可视化对象
        data_vis_dir = os.path.join(args.save_dir, args.dataset, 'data_vis', 'train')
        print("data_vis_dir,数据可视化路径： -------%s -------" % data_vis_dir)
        if not os.path.exists(data_vis_dir):
            os.makedirs(data_vis_dir)
        id_number_to_name = {}
        for r in roidb:
    ...
    # TODO:加载保存的模型
    # log_dir = "/content/Visual-Manipulation-Relationship-Network-Pytorch-pytorch1.0/data/mgn_4_6_12-16-1-48.pth"
    ...
    for epoch in range(args.checkepoch, args.max_epochs + 1):
        ...
        Network.train()
        print("epoch %d, learning_rate: %.6f" % (epoch, optimizer.state_dict()['param_groups'][0]['lr']))
        ...
        while (True):
            ...
            if args.frame == 'faster_rcnn_vmrn':
                ...
                # cls_prob：类预测概率
                # bbox_pred：目标框预测位置
                # rel_cls_prob：rel is what？
                # rpn_loss_cls：rpn检测的目标框种类损失
                # rpn_loss_box：rpn检测的目标框位置损失
                # loss_cls：分类损失
                # loss_bbox：目标位置损失
                # rel_loss_cls：rel is what？
                # reg_loss：回归损失是什么东西？
                # rois_label：？
                # grasp_loc：？
                # grasp_prob：？
                # grasp_bbox_loss：抓取位置损失
                # grasp_cls_loss,：抓取类别损失
                # grasp_conf_label：抓取标签置信度
                # grasp_all_anchors：？
        ...
        if epoch % 1 == 0:    
            # test network and record result
            ...
            if epoch % 2 == 0 and save_flag:
                # 模型保存路径
                # save_name = os.path.join(output_dir, args.frame + '_{}_{}_{}.pth'.format(args.session, epoch, step))
                localtime = time.localtime(time.time())
                localtime = localtime[1:5]
                time_str = str(localtime[0]) + '-' + str(localtime[1]) + '-' + str(localtime[2]) + '-' + str(
                  localtime[3])
                output_dir = "/content/drive/My Drive/model"  # colab路径

                save_name = os.path.join(output_dir, args.frame + '_{}_{}_{}.pth'.format(args.session, epoch, time_str))
                save_checkpoint({
